---
title: "Analysis"
description: "This page details the models we employed to explore the relationship between income and rent, along with discussions on the models' reliability and validity."
toc: true
draft: FALSE
---
...
data_path <- here("dataset", "data_twenty_clean.rds")
data_path1 <- here("dataset", "latest.rds")
data_twenty <- readRDS(data_path) 
data_2022 <- readRDS(data_path1) 
```
...
data_wide <- data %>%
  pivot_wider(
    ...
    values_fn = list(estimate = max)  # Use max to ensure that we take the non-NA values if they exist
  ) %>%
  # Ensure all necessary columns are correctly named and handled
  rename_with(~tolower(gsub(" ", "_", .x)), .cols = everything()) %>%
  # Aggregate data if necessary (seems like you might be working with other demographic data as well)
  group_by(geoid, name) %>%
  summarise(
    median_income = max(median_income, na.rm = TRUE),
    median_rent = max(median_rent, na.rm = TRUE),
...
# Join ACS data with state geometries
data_geo <- left_join(states, data_wide, by = c("GEOID" = "geoid"))

# Function to create a plot for a given variable
plot_data <- function(data, value_column, title) {
  ggplot(data) +
    geom_sf(aes(fill = !!sym(value_column)), color = "white", size = 0.1) +
    scale_fill_viridis_c(option = "magma", direction = -1, name = title, guide = "none") +
    labs(
      title = title, 
      subtitle = "Data from ACS 2019 5-year estimates"
    ) +
    theme_minimal() +
    theme(
...
plot_rent <- plot_data(data_geo, "median_rent", "Median Rent by State")
plot_income <- plot_data(data_geo, "median_income", "Median Income by State")

# Combine the plots using patchwork
combined_plot <- plot_rent + plot_income + plot_layout(nrow = 1)
print(combined_plot)
```
...
1.  Multiple Linear Regression Model [(Detailed in Class Blog 4)](https://sussmanbu.github.io/final-project-team9/posts/2024-03-30-class-blog-4/class-blog-4.html)


2.  Log-Log Regression Model[(Detailed in Class blog 4)](https://sussmanbu.github.io/final-project-team9/posts/2024-03-30-class-blog-4/class-blog-4.html)


3.  Log-Log Regression Model with interaction terms [(Detailed in Class blog 4)](https://sussmanbu.github.io/final-project-team9/posts/2024-03-30-class-blog-4/class-blog-4.html)
...
```
-   A review of multicollinearity prompted the removal of `FTOTINC`(family income) to prevent model overfitting.
...
-   `Employment Status (EMPSTAT)`, although not as critical, also showed a non-significant p-value and is suspected to interact with variables like `Income (INCTOT)` or `Family_income(FTOTINC)`. This interaction will be explored further.
...
- The model suggests that a **10% increase in income is predicted to raise rent by an average of 1.334% (defined as elasticity in economic terms)**, with all other variables held constant.
...
# Set up a multi-panel layout with two columns
par(mfrow = c(1, 2))
...
-   The interaction term `EMPSTAT * FTOTINC (employment status * family income)` has been introduced to investigate the suspected interaction effect.
...
- But the p-value for `EMPSTAT Unemployed:FTOTINC (umployment status * family income)` is slightly above 0.05, suggesting it has borderline statistical significance. Therefore, adding interaction term **needs to have further consideration**.
...
1.  Time series model [(Detailed in Class blog 7)](https://sussmanbu.github.io/final-project-team9/posts/2024-04-19-blog-post-7/blog-post-7.html)

2.  Log-Log Regression Model by YEAR [(Detailed in Class blog 5)](https://sussmanbu.github.io/final-project-team9/posts/2024-04-12-class-blog-5/class-blog-5.html)

3.  Longitudinal Log-Log Regression Model by Race [(Detailed in Class blog 6)](https://sussmanbu.github.io/final-project-team9/posts/2024-04-17-class-blog-6/class-blog-6.html)
...
- The model suggests that a **10% increase in income is predicted to raise rent by an average of 1.412%**, with all other variables held constant.
...
fitted_values <- fitted(model_with_income_log)
...
### Model Overview:
-   This model extended the 2022's one_year dataset analysis for race to explore how the rent-income relationship for each racial group has evolved from 2000 to 2022.
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
sampled_data <- data_twenty_filtered_final %>%
  group_by(REGION_CLASSIFIED, RACE) %>%
  sample_n(size = 1000, replace = TRUE) %>%  
  ungroup()

rent_summary <- sampled_data %>%
  group_by(REGION_CLASSIFIED, RACE) %>%
  summarize(total_rent = sum(RENTGRS), .groups = 'drop')

region_totals <- rent_summary %>%
  group_by(REGION_CLASSIFIED) %>%
  summarize(region_total_rent = sum(total_rent), .groups = 'drop')

rent_summary <- rent_summary %>%
  left_join(region_totals, by = "REGION_CLASSIFIED") %>%
  mutate(percent = (total_rent / region_total_rent) * 100) %>%
  arrange(REGION_CLASSIFIED, desc(percent))  

ggplot(rent_summary, aes(x = REGION_CLASSIFIED, y = percent, fill = RACE)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(aes(label = sprintf("%.1f%%", percent)), position = position_stack(vjust = 0.5), size = 2.5, color = "black") +
  scale_fill_viridis_d(option = "magma", begin = 0.9, end = 0.3) +  # Invert colors for better visual representation
  labs(title = "Percentage of Rental Expenditure (Sampled 1000 Data from each race)",
       x = "Region",
...
1. **Income and Rent Expenditure (2022):** Latest data indicates that a 10% increase in income is predicted to raise rent expenditure by approximately 1.334%, with other variables constant, showcasing the elastic nature of rent in response to changes in income.

2. **Time Effect and Autocorrelation Considerations:** When accounting for temporal effects and autocorrelation, a 10% increase in income is expected to lead to a 1.467% rise in rent, affirming the pronounced elasticity of rent costs relative to income fluctuations.

3. **Trends Over Years:** Analysis of yearly data reveals that coefficient estimates peaked notably between after 2000 and before 2020, followed by declines during the COVID-19 period, indicating cycles of increase and corrective adjustments in the relationship between income and rent.
