...
-   **Quantitative Analysis**: The goal is to quantify the relationship precisely. For example, we seek to estimate the increase in rental expenditure for every thousand-dollar increase in income or the percentage change of rent expenditure for every one percent increase in income.
...
1.  Multiple Linear Regression Model (Detailed in Class Blog 4)

2.  Log-Log Regression Model (Detailed in Class blog 4)

3.  Log-Log Regression Model with interaction terms (Detailed in Class blog 4)
...
-   The outliers was excluded (three standard deviations away from the response and predictor variable) for cleansing.
...
1.  Time series model (Detailed in Class blog 7)

2.  Log-Log Regression Model by YEAR (Detailed in Class blog 5)

3.  Longitudinal Log-Log Regression Model by Race (Detailed in Class blog 6)
...
- The model suggests that a **10% increase in income is predicted to raise rent by an average of 14.12%**, with all other variables held constant, indicating income's elasticity effect on rent.
...
```
- From the plot, we sampled 1000 individuals from each racial group in various regions to analyze their impact on rental expenditures. The visualization reveals significant variations in rental spending across races within each region, emphasizing disparities that are vital for guiding regional housing policies and economic strategies.
