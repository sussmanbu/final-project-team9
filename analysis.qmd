...
### Model Overview:
<<<<<<< HEAD
*******Why use time series model here, how it improved upon the previos one;

### Model Findings:
=======
- Here we use time series random effect model to further analyze the relationship between rent and income. 
...
>>>>>>> 5c8d869ac73f418ba2a662e1a710dd4aa7312d4b
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
### add a table here for the conclsuion: broom:tidy 
```
...
```
- The model suggests that a 10% increase in income is predicted to raise rent by an average of 14.67%, with all other
variables held constant, indicating income's elasticity effect on rent.
...
```
<<<<<<< HEAD
- Residual Standard Error (sigma): The residual standard error is 0.4329367, This value indicates there are deviation of the observed values from the fitted values. 
...
>>>>>>> 6e77506c9cc146911bce2b216819e0975cb5e29a
=======
- Lower standard errors indicate more precise estimates of the coefficients, and most predictors have relatively low standard errors, suggesting a high degree of precision in the estimates. 
...
=======
- The residual plot seems to have certain problem with heteroscedasticity, which is pretty normal in our model since we encountered similar problem in blog post 4. 
>>>>>>> 5c8d869ac73f418ba2a662e1a710dd4aa7312d4b
