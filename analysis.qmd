...
log_inctot_p_values <- data_twenty %>%
  group_by(RACE, YEAR) %>%
  filter(if_all(c(KITCHEN, ROOMS, NFAMS, AGE, MARST, EMPSTAT, REGION_CLASSIFIED), ~n_distinct(.) > 1)) %>% 
  do({
    ...
    if (!is.null(safely_result)) {
      ...
      data.frame(YEAR = first(.$YEAR), RACE = first(.$RACE), term = NA, estimate = NA, p.value = NA) 
    }
  }) %>%
  ungroup() %>%
