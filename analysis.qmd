...
>>>>>>> 48e394924c6c7a414a4e0e2a4b42f69a9df6f7b7
...
=======
### Model overview:
- This model extended the 2022 dataset analysis to explore how the rent-income relationship have evolved other the past tenty years
- We use random effect time series model which (1 | REGION_CLASSIFIED) term introduces a random intercept for different regions (REGION_CLASSIFIED), allowing the model to account for variations in rent prices that are region-specific but not explained by the model's fixed effects.
- The primary goal is to understand how changes in income (on a logarithmic scale) and other demographic and socio-economic factors affect the log-transformed rent prices, adjusting for regional variations.
...
```
- The model suggests that a 10% increase in income is predicted to raise rent by an average of 16.09%, with all other variables held constant, indicating income's elasticity effect on rent.
...
kable(glance_lm, format = "pipe")
```
- There are 31662 observations in the table, which generally is a large number which is good. 
- Residual Standard Error (sigma): The residual standard error is 0.4329367, This value indicates there are deviation of the observed values from the fitted values. A lower residual standard error suggests a model with a good fit to the data.
- Log-Likelihood (logLik): The log-likelihood of the model is -18433.34. Log-likelihood measures how well the model predicts the observed data; higher values (closer to zero) typically indicate a better model fit.
...
=======
>>>>>>> f4b8d7617dd335bd88f7ff14da0f40baa12d128e
