...
server <- function(input, output, session) {
  output$modelTable <- renderDT({
    ...
    # Assuming 'tidied' contains complex objects, let's extract only necessary information
    # For example, assuming 'tidied' contains coefficient estimates:
    year_data <- year_data %>%
      mutate(
        estimate = map_dbl(tidied, ~ .x$estimate[.x$term == "log_INCTOT"]),  # Extracting estimates for 'log_INCTOT'
        std_error = map_dbl(tidied, ~ .x$std.error[.x$term == "log_INCTOT"]),  # Extracting std.error
        statistic = map_dbl(tidied, ~ .x$statistic[.x$term == "log_INCTOT"]),  # Extracting t-value
        p_value = map_dbl(tidied, ~ .x$p.value[.x$term == "log_INCTOT"])  # Extracting p-value
      ) %>%
      select(YEAR, estimate, std_error, statistic, p_value)  # Selecting only necessary columns

    # Render the table with simpler data
    datatable(year_data, 
      options = list(
        pageLength = 1,      
...
-   All showed to be statistically significant \### Discussion of the topic: Race is really a point that needs to explore. Thorugh the result and the visualization of the coefficient changes for each race, there is defintely different pattens showed.
