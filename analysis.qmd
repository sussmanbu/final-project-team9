...
models_with_stats <- data_twenty_filtered_final %>%
  group_by(RACE, YEAR) %>%
  do({
    model = lm(log_RENTGRS ~ log_INCTOT + KITCHEN + ROOMS + NFAMS + AGE + MARST + EMPSTAT + REGION_CLASSIFIED, data = .)
    tidy(model)  # This function from broom package automatically formats model output
  }) %>%
  ungroup()  # Ensures we can manipulate the overall data frame

# Flatten the results to have a single row per RACE-YEAR with coefficients and p-values
models_flat <- models_with_stats %>%
  pivot_wider(names_from = term, values_from = c(estimate, p.value), names_sep = "_")
