...
```{r echo=FALSE, message=FALSE, warning=FALSE}
data_path <- here("dataset", "latest_clean.csv")
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
rent_data_clean <- rent_data_clean[rent_data_clean$RENTGRS > 0 & rent_data_clean$INCTOT > 0, ]
rent_data_clean$log_RENTGRS <- log(rent_data_clean$RENTGRS)
rent_data_clean$log_INCTOT <- log(rent_data_clean$INCTOT)
enhanced_model_augmented <- lm(log_RENTGRS ~ log_INCTOT + KITCHEN + ROOMS + NFAMS + AGE + MARST + RACE + EMPSTAT + REGION_CLASSIFIED, data = rent_data_clean)


ggplot(enhanced_model_augmented, aes(x = log_INCTOT, y = log_RENTGRS)) + stat_density2d_filled() + scale_x_log10() + scale_y_log10()

ggplot(enhanced_model_augmented, aes(x = log_INCTOT, y = log_RENTGRS)) + geom_point(alpha =.05) + scale_x_log10() + scale_y_log10()
```
...
ggplot(log_model_2022, aes(x = log_INCTOT, y = log_RENTGRS)) + geom_point(alpha =.05) + scale_x_log10() + scale_y_log10()

ggplot(log_model_2022, aes(x = log_INCTOT, y = log_RENTGRS)) + stat_density2d_filled() + scale_x_log10() + scale_y_log10()
```
